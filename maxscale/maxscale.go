...
func New(maxInfoAddress string) (*MaxScale, error) {
	return &MaxScale{
		maxInfoAddress: maxInfoAddress,
	}, nil
}

func (m *MaxScale) url(path string) (string) {
	return "http://" + m.maxInfoAddress + path
}

func (m *MacScale) servers() (string){
	url := m.url("/servers")
	fmt.Println(url)
